...
from .audio_dac import CONF_PCM5122_ID, pcm5122_ns, Pcm5122Component

ExcludeFromHaveFault = pcm5122_ns.enum("ExcludeFromHaveFault")
...
async def to_code(config):
    cg.add_define("USE_PCM5122_BINARY_SENSOR")
    ...
    if has_fault_config := config.get(CONF_HAVE_FAULT):
        ...
        cg.add(pcm5122_component.set_have_fault_binary_sensor(sens))
        cg.add(pcm5122_component.config_exclude_fault(config[CONF_HAVE_FAULT][CONF_EXCLUDE]))

    if has_fault_config := config.get(CONF_LEFT_CHANNEL_DC_FAULT):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_left_channel_dc_fault_binary_sensor(sens))

    if has_fault_config := config.get(CONF_RIGHT_CHANNEL_DC_FAULT):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_right_channel_dc_fault_binary_sensor(sens))

    if has_fault_config := config.get(CONF_LEFT_CHANNEL_OVER_CURRENT):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_left_channel_over_current_fault_binary_sensor(sens))

    if has_fault_config := config.get(CONF_RIGHT_CHANNEL_OVER_CURRENT):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_right_channel_over_current_fault_binary_sensor(sens))

    if has_fault_config := config.get(CONF_OTP_CRC_ERROR):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_otp_crc_check_error_binary_sensor(sens))

    if has_fault_config := config.get(CONF_BQ_WRITE_FAILED):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_bq_write_failed_fault_binary_sensor(sens))

    if has_fault_config := config.get(CONF_CLOCK_FAULT):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_clock_fault_binary_sensor(sens))

    if has_fault_config := config.get(CONF_PVDD_OVER_VOLTAGE):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_pvdd_over_voltage_fault_binary_sensor(sens))

    if has_fault_config := config.get(CONF_PVDD_UNDER_VOLTAGE):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_pvdd_under_voltage_fault_binary_sensor(sens))

    if has_fault_config := config.get(CONF_OVER_TEMP_SHUTDOWN):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_over_temperature_shutdown_fault_binary_sensor(sens))

    if has_fault_config := config.get(CONF_OVER_TEMP_WARNING):
        sens = await binary_sensor.new_binary_sensor(has_fault_config)
        cg.add(pcm5122_component.set_over_temperature_warning_binary_sensor(sens))
